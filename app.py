...
# longest monster name is Sergeant Titanica (17 characters)
...
def gen_image(lp1name, lp1health, lp1state,
    ...
    lp = [{'name': lp1name, 'health': lp1health, 'state': det_state(lp1health, lp1state)},
          {'name': lp2name, 'health': lp2health, 'state': det_state(lp2health, lp2state)},
          {'name': lp3name, 'health': lp3health, 'state': det_state(lp3health, lp3state)}]
    lp = [m for m in lp if m['name'] != '']
    gen_monster_img(lp, 'left')
